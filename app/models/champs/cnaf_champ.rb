...
class Champs::CnafChamp < Champs::TextChamp
  ...
  validates :numero_allocataire, format: { with: /\A\d{1,7}\z/ }, if: -> { code_postal.present? && should_validate_in_current_context? }
  validates :code_postal, format: { with: /\A\w{5}\z/ }, if: -> { numero_allocataire.present? && should_validate_in_current_context? }
